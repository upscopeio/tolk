...
module Tolk
  class Locale < ApplicationRecord
    ...
    self.locales_config_path = _dump_path
    ...
    PLURALIZATION_KEYS = %w[none zero one two few many other]

    class << self
      def primary_locale(reload = false)
        @_primary_locale = nil if reload
        @_primary_locale ||= begin
          unless primary_locale_name
            raise "Primary locale is not set. Please set Locale.primary_locale_name in your application's config file"
          end

          where(name: primary_locale_name).first_or_create
        end
      end
      ...
      def special_key_or_prefix?(prefix, key)
        special_prefixes.include?(prefix) || special_keys.include?(key)
      end
    ...
    def dump(to = locales_config_path, exporter = Tolk::Export)
    ...
    def search_phrases_without_translation(query, page = nil)
      ...
      existing_ids = translations.select('tolk_translations.phrase_id').to_a.map(&:phrase_id).uniq
      ...
      if existing_ids.present?
        phrases = phrases.where(['tolk_phrases.id NOT IN (?) AND tolk_phrases.id IN(?)', existing_ids,
    ...
    def language_name
      Tolk.config.mapping[name] || name
    end

    def get(key)
      return unless phrase = Tolk::Phrase.where(key: key).first

      t = translations.where(phrase_id: phrase.id).first
      t.text if t
    end
    ...
    def self.rename(old_name, new_name)
      ...
      elsif locale = where(name: old_name).first
    ...
    def remove_invalid_translations_from_target
      translations.target.dup.each do |t|
        if t.valid?
          t.updated_at_will_change!
        else
          translations.target.delete(t)
        end
      end
